...
class PriorityQueue {
  ...
  heapifyUp() {
    ...
    while (curIdx > 0) {
      var parIdx = Math.floor(curIdx - 1 / 2); // 여기 (curIdx-1)
  ...
  heapifyDown() {
    ...
    while (true) {
      ...
      if (
        leftChild < this.size() &&
        this.heap[smallestIdx] > this.heap[leftChild]
      ) {
        smallestIdx = leftChild;
      }
      if (
        rightChild < this.size() &&
        this.heap[smallestIdx] > this.heap[rightChild]
      ) {
        smallestIdx = rightChild;
      }
      if (curIdx === smallestIdx) break;
      [this.heap[smallestIdx], this.heap[curIdx]] = [
        this.heap[curIdx],
...
function solution(scoville, K) {
  ...
  if (pq.dequeue() === null || pq.dequeue() < K) return -1; // 여기서 dequeue dequeue 해버리면 안됨
  return answer;
}
